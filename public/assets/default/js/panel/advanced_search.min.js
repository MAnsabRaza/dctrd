(function ($) {
    ...
    /* ═══════════════════════════════════════════════════════════
       5. LIVE SUGGESTIONS  (TP5) — FIX-JS-1 applied
    ═══════════════════════════════════════════════════════════ */
    ...
    $input.on('keyup', function (e) {
        // Skip navigation keys — don't re-trigger AJAX on arrow/enter
    ...
    /* ═══════════════════════════════════════════════════════════
       6. NEARBY / CITY GEOCODING + GPS  (TP6, TP8)
         FIX-JS-2 applied throughout
    ═══════════════════════════════════════════════════════════ */
    ...
    function clearLatLng() {
        $latField.val('');
        $lngField.val('');
        $cityHidden.val('');  // FIX-JS-2
        $clearCity.hide();
    }

    // GPS "Use my location" button — FIX-JS-2: cityHidden updated via reverse geocode
            ...
            function (pos) {
                ...
                $.getJSON(
                    ...
                    function (data) {
                        ...
                        if (city) {
                            $cityInput.val(city);
                            setLatLng(lat, lng, city);  // FIX-JS-2
                        } else {
                            setLatLng(lat, lng, '');
                        }
    ...
    // City text → Nominatim geocode — FIX-JS-2: cityHidden synced
    $cityInput.on('input', function () {
        var city = $(this).val().trim();
        clearTimeout(geocodeTimer);
        $clearCity.toggle(city.length > 0);

        // FIX-JS-2: always keep hidden city in sync with visible text
        $cityHidden.val(city);
            ...
            $.getJSON(
                ...
                function (results) {
                    if (results && results[0]) {
                        setLatLng(results[0].lat, results[0].lon, city);  // FIX-JS-2
                    }
                }
            );
        }, 500);
    ...
    // Clear city — FIX-JS-2: cityHidden cleared inside clearLatLng()
    $clearCity.on('click', function () {
        $cityInput.val('');
        clearLatLng();
    });

    // Show clear button if lat already in URL
    if ($latField.val()) { $clearCity.show(); }

    // IP-based auto-detect (only when no lat set yet) — FIX-JS-2: cityHidden set
    if (!$latField.val() && $cityInput.length && !$cityInput.val()) {
        $.getJSON('https://ip-api.com/json/?fields=city,lat,lon', function (data) {
            if (data && data.city && !$cityInput.val()) {
                $cityInput.val(data.city);
                setLatLng(data.lat, data.lon, data.city);  // FIX-JS-2
            }
        }).fail(function () {});
    }
    ...
    /* ═══════════════════════════════════════════════════════════
       7. FORM SUBMIT — strip empty lat/lng for clean URL
         FIX-JS-3 applied: types[] handling fixed
    ═══════════════════════════════════════════════════════════ */

    $form.on('submit', function () {
        // FIX-JS-3: Only disable if SOME are unchecked (not all unchecked)
        // When all are checked = same as no filter (search all) — disable all = clean URL
        // When some are checked = keep only checked ones
        ...
        if ($checked.length === 0 || $checked.length === $allTypes.length) {
            // All or none selected — disable all, let controller search everything
            $allTypes.prop('disabled', true);
        }
        // else: some specific types selected — leave them enabled, unchecked ones
        // won't appear in URL anyway since they're unchecked form fields

        // Disable booking_categories if all/none checked (same logic)
    ...
    // Type tabs (All / Bookings / Courses / …)
    ...
    // Results page sidebar filter form — category checkboxes
    ...
    // ── Results page sidebar: city geocoding — FIX-JS-4 applied ─────────────
    ...
    $sidebarCity.on('input', function () {
        ...
        // FIX-JS-4: keep city field in sync as user types
            ...
            $.getJSON(
                ...
                function (results) {
                    if (results && results[0]) {
                        ...
                        if ($sidebarCityFld.length) $sidebarCityFld.val(city);  // FIX-JS-4
                    }
                }
            );
        }, 500);
    ...
    // IP auto-detect for results page sidebar (when no lat in URL)
    if ($sidebarLat.length && !$sidebarLat.val() && $sidebarCity.length && !$sidebarCity.val()) {
        $.getJSON('https://ip-api.com/json/?fields=city,lat,lon', function (data) {
            if (data && data.city && !$sidebarCity.val()) {
                $sidebarCity.val(data.city);
                if ($sidebarLat.length)     $sidebarLat.val(data.lat);
                if ($sidebarLng.length)     $sidebarLng.val(data.lon);
                if ($sidebarCityFld.length) $sidebarCityFld.val(data.city);  // FIX-JS-4
            }
        }).fail(function () {});
    }
