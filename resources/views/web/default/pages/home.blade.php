...
@push('scripts_bottom')
    ...
    <script>
        ...
        {
            const languagePopup = @json($languagePopup);
            console.log('Language Popup Data:', languagePopup);
            if (languagePopup) 
            {
                // Prefer language_code if available
                const languageCode = languagePopup.language_code || 'EN';
                console.log('Mapped Language Code:', languageCode);
                if (languagePopup.popup.action_type == 1) 
                {
                    // Show popup
                    showLanguagePopup(
                        languagePopup.country,
                        languagePopup.language,
                        languagePopup.popup.notification_title,
                        languagePopup.popup.notification_text,
                        languagePopup.popup.confirm_button_text,
                        languagePopup.popup.cancel_button_text,
                        languageCode
                    );
                } else if (languagePopup.popup.action_type == 0) {
                    // Auto-redirect via POST
            ...
            // +++++++++++++++++ showLanguagePopup() +++++++++++++++++
            function showLanguagePopup(country, language, title, text, confirmText, cancelText, languageCode)
            {
                let popup = `
                    <div id="languagePopup" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="languagePopupTitle" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="languagePopupTitle">${title}</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">×</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <p>${text.replace('{country}', country || 'Unknown').replace('{language}', language)}</p>
                                </div>
                                <div class="modal-footer">
                                    <button id="confirmLanguage" class="btn btn-primary">${confirmText}</button>
                                    <button id="cancelLanguage" class="btn btn-secondary" data-dismiss="modal">${cancelText}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                $('#confirmLanguage').click(function() {
                    setLanguage(languageCode);
                    $('#languagePopup').modal('hide');
                });
                // Remove modal on close
                $('#languagePopup').on('hidden.bs.modal', function() {
                    $(this).remove();
                });
            }
            // ++++++++++++++++ setLanguage() ++++++++++++++++
            function setLanguage(locale)
            {
                ...
                console.log('AJAX URL:', '{{ route('appLocaleLanguageOverIpRoute') }}');
                $.ajax({
                    url: '{{ route('appLocaleLanguageOverIpRoute') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        locale: locale,
                    ...
                    {
                        console.log('Set language response:', response);
                        if (response.success) 
                        {
                            // Set cookie for frontend confirmation (optional, as backend sets it)
                            document.cookie = `preferred_language=${locale}; path=/; max-age=${30 * 24 * 60 * 60}`;
                            $('#languagePopup').modal('hide');
                            window.location.href = response.redirect_url || window.location.href;
                        } else {
                            console.error('Language change failed:', response.message);
                            $('#languagePopup').modal('hide');
                            alert('Failed to change language: ' + (response.message || 'Unknown error'));
                        }
                    },
                    error: function(xhr) {
                        ...
                        $('#languagePopup').modal('hide');
                        alert('Failed to change language: ' + (xhr.responseJSON?.message || 'Server error'));
                    }
                });
            }
        });
