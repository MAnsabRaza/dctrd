...
@if($user->isOrganization())
    @php
        ...
        $frontendLink = $locale === 'el' ? $user->frontend_link_gk : $user->frontend_link;
    @endphp
 <section>
    <div class="panel-section">
        @if(!empty($frontendLink) && filter_var($frontendLink, FILTER_VALIDATE_URL))
            <iframe src="{{ $frontendLink }}" width="100%"
                height="{{ !empty($user) && !empty($user->front_iframe_height) ? $user->front_iframe_height : '600' }}px"
                style="border: none;"></iframe>
        @else
            <p style="margin-top: 30px">No Data Found Yet.
