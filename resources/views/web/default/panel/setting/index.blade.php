    ...
    <form method="post" id="userSettingForm" class="mt-30"
        ...
        @if (empty($new_user) and !empty($currentStep))
            @switch($currentStep)
                ...
                @case(9)
                    @include('web.default.panel.setting.setting_includes.backend_iframe_connection')
                @break

                {{-- Cross Settings --}}
                @case(10)
                    @if ($authUser->cross_selling)
                        @include('web.default.panel.setting.setting_includes.cross-selling')
                    @endif
                @break
                ...
                @case(11)
                    @if ($authUser->up_selling)
                        @include('web.default.panel.setting.setting_includes.up-selling')
                    @endif
                @break
            @endswitch
        @endif
