                ...
                <div class="col-12 col-lg-6">
                    <div class="row">
                        ...
                        <div class="col-12 col-lg-4">
                            <div class="form-group">
                                <label class="input-label d-block">Courses / Products</label>
                                <select name="item_id" id="item_id"
                                        class="form-control custom-select @error('item_id')  is-invalid @enderror">
                                    <option selected disabled>Select</option>
                                    {{-- ======= Webinars (Courses) ======= --}}
                                    <optgroup label="Courses">
                                        @foreach($webinars as $webinar)
                                            <option value="course_{{ $webinar->id }}">{{ $webinar->title }}</option>
                                        @endforeach
                                    </optgroup>
                                    {{-- ======= Products ======= --}}
                                    <optgroup label="Products">
                                        @foreach($products as $product)
                                            <option value="product_{{ $product['id'] }}">{{ $product['title'] }}</option>
                                        @endforeach
                                    </optgroup>
                                </select>
                        ...
                        <div class="table-responsive">
                            <table class="table custom-table text-center">
                                ...
                                <tr>
                                    ...
                                    <th class="text-left text-gray">Item</th>
                                    <th class="text-left text-gray">Discount Type</th>
                                ...
                                @foreach($specialOffers as $specialOffer)
                                    <tr>
                                        ...
                                        {{-- ++++++++++++ item : product or course ++++++++++++ --}}
                                        <td class="text-left align-middle">
                                            @if($specialOffer->webinar)
                                                <a href="{{ $specialOffer->webinar->getUrl() }}" class="text-dark-blue font-weight-500" target="_blank">
                                                    {{ $specialOffer->webinar->title }}
                                                </a>
                                            @elseif($specialOffer->product)
                                                <a href="{{ $specialOffer->product->getUrl() }}" class="text-dark-blue font-weight-500" target="_blank">
                                                    {{ $specialOffer->product->title }}
                                                </a>
                                            @else
                                                <span class="text-danger">{{ trans('public.not_available') }}</span>
                                            @endif
                                        </td>
                                        {{-- ++++++++++++ discount_type ++++++++++++ --}}
                                        <td class="align-middle">
                                            <span class="text-dark-blue font-weight-500">{{ $specialOffer->discount_type }}</span>
                                        </td>
                                        <td class="align-middle">
