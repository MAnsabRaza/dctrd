...
@section('content')
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <ul class="nav nav-pills" id="myTab3" role="tablist">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link {{ ((!empty($errors) and count($errors)) or !empty($editFeaturedCategory) or (!empty($activeTab) and $activeTab == 'new') or (empty($featuredCategories) or !$featuredCategories->count())) ? 'active' : '' }}" id="newFeaturedCategory-tab" data-toggle="tab" href="#newFeaturedCategory" role="tab" aria-controls="newFeaturedCategory" aria-selected="true">{{ trans('update.add_top_category') }}</a>
                                </li>
                            </ul>
    ...
    <div class="btn-group dropdown table-actions position-relative">
        ...
        <div class="dropdown-menu dropdown-menu-right">
            @can('admin_booking_top_categories_edit')
                <a href="{{ getAdminPanelUrl() }}/booking/top-categories/{{ $featuredCategory->id }}/edit"
            ...
            @can('admin_booking_top_categories_delete')
                @include('admin.includes.delete_button',[
                    'url' => getAdminPanelUrl().'/booking/top-categories/'.$featuredCategory->id.'/delete',
                                ...
                                <div class="tab-pane mt-3 fade {{ ((!empty($errors) and count($errors)) or !empty($editFeaturedCategory) or (!empty($activeTab) and $activeTab == 'new') or (empty($featuredCategories) or !$featuredCategories->count())) ? 'active show' : '' }}" id="newFeaturedCategory" role="tabpanel" aria-labelledby="newFeaturedCategory-tab">
                                    <div class="row">
                                        <div class="col-12 col-md-6">
                                            <form action="{{ getAdminPanelUrl() }}/booking/top-categories/{{ !empty($editFeaturedCategory) ? $editFeaturedCategory->id.'/update' : 'store' }}" method="post">
                                                {{ csrf_field() }}
