            ...
            <div class="form-group">
                ...
                <select name="booking_id" class="form-control select2">
                    ...
                    @foreach($allBookingsLists as $allBookingList)
                        <option value="{{ $allBookingList->id }}" {{ request()->get('booking_id') == $allBookingList->id ? 'selected' : '' }}>{{ $allBookingList->title }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="col-12 col-lg-3">
            <div class="form-group">
                ...
                <select name="status" class="form-control select2">
                    <option value="">{{ trans('public.all') }}</option>
                    <option value="published" {{ request()->get('status') == 'published' ? 'selected' : '' }}>{{ trans('public.published') }}</option>
                    <option value="pending" {{ request()->get('status') == 'pending' ? 'selected' : '' }}>{{ trans('public.pending') }}</option>
                    <option value="draft" {{ request()->get('status') == 'draft' ? 'selected' : '' }}>{{ trans('public.draft') }}</option>
                </select>
            </div>
        </div>

        <div class="col-12 col-lg-3 ml-auto mt-16">
