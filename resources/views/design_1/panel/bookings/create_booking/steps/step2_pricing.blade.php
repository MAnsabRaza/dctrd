{{--
    Step 2 of 8 — Pricing & Availability
    Maps to Image 3: Booking Costs, seasonal Rate Type table, Booking Availability rules
--}}
@php
    $existingPlans = $booking->ratePlans ?? collect();
@endphp

<form data-wiz-form id="stepPricingForm">
    ...
    <div class="row">
        ...
        <div class="col-12 col-md-4" data-types="tour,activity,service">
            <div class="form-group">
        ...
        {{-- Org-level availability rule summary (read-only here, managed at org settings level) --}}
        @if($orgAvailabilityRule ?? null)
            <div class="col-12 mt-4">
                <div class="alert alert-light border">
                    ...
                    @if($orgAvailabilityRule->product_specific_takes_precedence)
                        <span class="badge badge-info ml-2">This booking's own ranges override the org default</span>
                    @endif
                </div>
            </div>
        @endif
...
(function () {
    ...
    // Show/hide fields per booking_type, based on the type chosen in Step 1
    const bookingType = document.getElementById('bookingWizardApp')?.dataset.bookingType;
    document.querySelectorAll('[data-types]').forEach(function (el) {
        const allowed = el.dataset.types.split(',');
        el.style.display = allowed.includes(bookingType) ? '' : 'none';
    });
})();
</script>
