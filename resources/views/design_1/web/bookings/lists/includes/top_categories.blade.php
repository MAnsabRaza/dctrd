        ...
        <div class="swiper-container js-make-swiper products-top-categories-slider pb-0 mx-16 mx-md-32"
            ...
            <div class="swiper-wrapper py-0">
                @foreach($topCategories as $topCategory)
                    @if(!empty($topCategory->category))
                        <div class="swiper-slide">
                            <a href="{{ $topCategory->category->getUrl() }}"
                               class="position-relative products-lists-top-category d-flex align-items-center p-12">
                                <div class="size-48 rounded-circle">
                                    <img src="{{ $topCategory->image }}"
                                         alt="{{ $topCategory->category->title }}"
                                         class="img-cover rounded-circle">
                                </div>
                                <div class="ml-8">
                                    <h5 class="font-14 text-dark">{{ $topCategory->category->title }}</h5>
                                    <p class="mt-4 font-12 text-gray-500">
                                        {{ trans('update.n_products', ['count' => $topCategory->category->bookings_count ?? 0]) }}
                                    </p>
                                </div>
                            </a>
                        </div>
