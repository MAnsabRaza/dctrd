                ...
                <div class="d-flex align-items-start justify-content-between gap-8">
                        ...
                        <div class="d-flex align-items-center flex-wrap gap-8 mt-6">
                            @if(!empty($booking->creator))
                                <a href="{{ $booking->creator->getProfileUrl() }}"
                                    ...
                                    <img src="{{ $booking->creator->getAvatar(20) }}" class="rounded-circle img-cover mr-4"
                                        style="width:18px;height:18px;" alt="{{ $booking->creator->full_name }}">
                    ...
                    <div class="d-flex flex-column align-items-end flex-shrink-0 gap-4">
                        <button type="button" class="cart-remove-btn" data-cart-id="{{ $itemKey }}"
                            ...
                            <x-iconsax-lin-close-circle class="icons text-red-600" width="18px" height="18px" />
                        </button>
                    ...
                    <div class="" data-item-key="{{ $itemKey }}">
                                ...
                                @if($daysModule)
                                    <div class="booking-info-card">
                                        ...
                                        <div style="display:flex;align-items:center;gap:10px;">
                                            ...
                                            <input type="date" name="{{ $datePrefix }}[check_in]"
                                                class="form-control form-control-sm bmod-date-input bmod-cin" value="{{ $oldCheckIn }}"
                                                min="{{ now()->format('Y-m-d') }}" style="border-radius:12px;flex:1;">
                                        </div>

                                        <div class="booking-info-label bmod-cin-label" style="margin-top:8px;font-size:13px;color:#64748b;">
                                            {{ $checkInLabel }}
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if($hoursModule)
                                    <div class="booking-info-card">
                                        ...
                                        <div style="display:flex;align-items:center;gap:10px;">
                                            ...
                                            <select name="{{ $timePrefix }}" class="form-control form-control-sm bmod-time-select"
                                        ...
                                        <div class="booking-info-label bmod-time-label"
                                    ...
                                    <div class="booking-info-card">
                                        ...
                                        <div style="display:flex;align-items:center;gap:10px;">
                                            ...
                                            @if(!empty($staffModule->config['staff']))
                                                <select name="{{ $staffPrefix }}" class="form-control form-control-sm"
                                                    style="border-radius:12px;flex:1;">
                                        ...
                                        <div class="booking-info-label" style="margin-top:8px;font-size:13px;color:#64748b;">
                                            {{ $staffLabel }}
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div class="d-flex flex-wrap gap-16">
                                    ...
                                    <div class="d-flex align-items-center gap-8">
                                        ...
                                        <div class="d-flex align-items-center gap-4">
                                            <button type="button" class="bmod-stepper-btn" data-target="pax_adults_{{ $itemKey }}"
                                                data-action="dec" data-min="{{ $adultMin }}">−</button>
                                            ...
                                            <button type="button" class="bmod-stepper-btn" data-target="pax_adults_{{ $itemKey }}"
                                                data-action="inc" data-max="{{ $adultMax }}">+</button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="d-flex align-items-center gap-8">
                                        ...
                                        <div class="d-flex align-items-center gap-4">
                                            <button type="button" class="bmod-stepper-btn" data-target="pax_children_{{ $itemKey }}"
                                                data-action="dec" data-min="{{ $childMin }}">−</button>
                                            <input type="number" id="pax_children_{{ $itemKey }}" name="{{ $paxPrefix }}[children]"
                                                value="{{ $oldChildren }}" min="{{ $childMin }}" max="{{ $childMax }}"
                                                class="bmod-stepper-input" readonly>
                                            <button type="button" class="bmod-stepper-btn" data-target="pax_children_{{ $itemKey }}"
                                                data-action="inc" data-max="{{ $childMax }}">+</button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="d-flex align-items-center gap-8">
                                        ...
                                        <div class="d-flex align-items-center gap-4">
                                            <button type="button" class="bmod-stepper-btn" data-target="pax_rooms_{{ $itemKey }}"
                                                data-action="dec" data-min="{{ $roomMin }}">−</button>
                                            <input type="number" id="pax_rooms_{{ $itemKey }}" name="{{ $paxPrefix }}[rooms]"
                                                value="{{ $oldRooms }}" min="{{ $roomMin }}" max="{{ $roomMax }}"
                                                class="bmod-stepper-input" readonly>
                                            <button type="button" class="bmod-stepper-btn" data-target="pax_rooms_{{ $itemKey }}"
                                                data-action="inc" data-max="{{ $roomMax }}">+</button>
                                        </div>
                                    </div>
                                ...
                                <div class="d-flex flex-wrap gap-8 mt-3">
                                    @foreach($extraOptions as $opt)
                                        <label class="d-flex align-items-center gap-6 px-10 py-8 rounded-12"
                                            ...
                                            <input type="checkbox" name="{{ $extrasPrefix }}[]" value="{{ $opt['label'] }}"
                                                data-price="{{ (float) ($opt['price'] ?? 0) }}" class="bmod-extra-chk"
                                                style="accent-color:#22c55e;" {{ in_array($opt['label'], old("checkout_modules.{$itemKey}.extra_services", [])) ? 'checked' : '' }}>
                                            <span>{{ $opt['label'] }}</span>
                        ...
                        @if($policyModule = $activeModules->firstWhere('name', 'cancellation_policy'))
                            ...
                            <div class="booking-cancellation-card">
                                <label for="cp_agree_{{ $itemKey }}" class="d-flex align-items-center gap-8" style="cursor:pointer;">
                                    <input type="checkbox" id="cp_agree_{{ $itemKey }}" name="{{ $policyPrefix }}" value="1" {{ old("checkout_modules.{$itemKey}.cancellation_policy") ? 'checked' : '' }}>
                        ...
                        @if($msgModule = $activeModules->firstWhere('name', 'checkout_message'))
                            ...
                            <div class="booking-cancellation-card">
                                ...
                                <textarea name="{{ $msgPrefix }}" class="form-control mt-3" rows="3"
                                    style="font-size:13px;border-radius:12px;min-height:88px;" placeholder="{{ $ph }}"
                                    maxlength="{{ $maxLen }}" id="bmod_msg_{{ $itemKey }}" {{ $msgModule->is_active ? '' : 'disabled' }}>{{ old("checkout_modules.{$itemKey}.checkout_message") }}</textarea>
                                <div style="font-size:11px;color:#94a3b8;margin-top:8px;">
                                    <span
                                        id="bmod_msgc_{{ $itemKey }}">{{ strlen(old("checkout_modules.{$itemKey}.checkout_message", '')) }}</span>/{{ $maxLen }}
                                </div>
                            </div>
                        @endif
                        ...
                        @if($revModule = $activeModules->firstWhere('name', 'reviewer_message'))
                            ...
                            <div class="booking-cancellation-card">
                                ...
                                <textarea name="{{ $revPrefix }}" class="form-control mt-3" rows="3"
                                    style="font-size:13px;border-radius:12px;min-height:88px;" placeholder="{{ $revPh }}"
                                    maxlength="{{ $revMaxLen }}"
                                    id="bmod_rev_{{ $itemKey }}">{{ old("checkout_modules.{$itemKey}.reviewer_message") }}</textarea>
                                <div style="font-size:11px;color:#94a3b8;margin-top:8px;">
                                    <span
                                        id="bmod_revc_{{ $itemKey }}">{{ strlen(old("checkout_modules.{$itemKey}.reviewer_message", '')) }}</span>/{{ $revMaxLen }}
                                </div>
                            </div>
                        @endif
    ...
    @else
        <div class="cart-item-card" id="cart-item-{{ $itemKey }}">
            <div class="flex-shrink-0">
                <img src="{{ $itemInfo['imgPath'] ?? '' }}" alt="{{ $itemInfo['title'] ?? '' }}" class="rounded-12 img-cover"
