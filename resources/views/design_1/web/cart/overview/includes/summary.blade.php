<div class="cart-summary-card card-with-mask position-relative">
    ...
    <div class="position-relative z-index-2 bg-white rounded-16 p-16 p-lg-20 w-100 h-100">
        ...
        <div class="cart-summary-row d-flex align-items-center justify-content-between mt-20">
            ...
            <span class="js-cart-subtotal js-cart-summary-amount" data-amount="{{ $calculatePrices['sub_total'] }}">{{ handlePrice($calculatePrices["sub_total"]) }}</span>
        </div>
        ...
        <div class="cart-summary-row d-flex align-items-center justify-content-between mt-16">
            ...
            <span class="js-cart-tax js-cart-summary-amount" data-amount="{{ $calculatePrices['tax_price'] ?? 0 }}">{{ !empty($calculatePrices["tax_price"]) ? handlePrice($calculatePrices["tax_price"]) : 0 }}</span>
        </div>

        @if(!empty($calculatePrices["product_delivery_fee"]))
            <div class="cart-summary-row d-flex align-items-center justify-content-between mt-16">
                <span class="text-gray-500">{{ trans('update.delivery_fee') }}</span>
                <span class="js-cart-delivery_fee js-cart-summary-amount" data-amount="{{ $calculatePrices['product_delivery_fee'] }}">{{ handlePrice($calculatePrices["product_delivery_fee"]) }}</span>
            </div>
        @endif
