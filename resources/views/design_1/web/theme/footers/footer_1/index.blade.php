@if(!empty($themeFooterData['contents']))
    @php
        ...
        // Detect public user profile pages like /users/light-moon/profile
        $isUserProfile = request()->is('users/*/profile');
    @endphp

    <div class="theme-footer-1 position-relative {{ $themeFooterHasNewsletter ? 'has-newsletter' : '' }} js-profile-footer-toggle-target" @if($isUserProfile) style="display: none;" @endif>
    ...
    @if($isUserProfile)
        <button id="toggleProfileFooter"
                type="button"
                title="Show / hide footer"
                aria-label="Show / hide footer"
                style="
                    position: fixed;
                    bottom: 25px;
                    right: 25px;
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                var toggleButton = document.getElementById('toggleProfileFooter');
                var footerTargets = document.querySelectorAll('.js-profile-footer-toggle-target');

                if (toggleButton && footerTargets.length) {
