...
@if(!empty($unitPreferences))
    <div class="js-unit-select theme-header-1__dropdown position-relative">
        <form action="{{ route('unitPreferences.update') }}" method="post">
            ...
            <div class="size-32 d-flex-center bg-white-10 rounded-8 cursor-pointer" title="Unit Preferences">
                <x-iconsax-lin-setting-2 class="icons text-white opacity-75" width="18px" height="18px"/>
            </div>
        </form>
            ...
            @foreach($unitPreferences as $unitType => $units)
                @if(!empty($units))
                    ...
                    @foreach($units as $unit => $label)
                        <div class="js-unit-dropdown-item header-1-dropdown-menu__item cursor-pointer {{ ($selectedUnits[$unitType] == $unit) ? 'active' : '' }}"
